...
namespace Project_Lions
{
    public static class BankSystem
    {
        ...
        {
            if (user.Username.Equals(userTry, StringComparison.InvariantCultureIgnoreCase))
            {
                if (!user.LockedOut)
                {
        ...
        {
            string logo = System.IO.File.ReadAllText(@"C:\Users\Lukas\OneDrive\Dokument\LionLogo.txt");
